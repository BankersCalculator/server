...
public class DtiCalcApiControllerDocsTest extends RestDocsSupport {
    ...
    void calculateDti() throws Exception {
        ...
        DtiCalcResponse response = DtiCalcResponse.builder()
                ...
                .totalLoanCount(1)
                .dtiCalcResultList(Arrays.asList(
                ...
                .andDo(document("calculator/dti-calc",
                        ...
                        responseFields(
                                ...
                                fieldWithPath("data.totalLoanCount").type(JsonFieldType.NUMBER)
                                        .description("총 대출 건수"),
                                fieldWithPath("data.dtiCalcResultList").type(JsonFieldType.ARRAY)
                                        .description("DTI 계산 결과 목록"),
                                fieldWithPath("data.dtiCalcResultList[].serial").type(JsonFieldType.NUMBER)
                                        .description("일련번호"),
                                fieldWithPath("data.dtiCalcResultList[].principal").type(JsonFieldType.NUMBER)
                                        .description("원금"),
                                fieldWithPath("data.dtiCalcResultList[].balance").type(JsonFieldType.NUMBER)
                                        .description("잔액"),
                                fieldWithPath("data.dtiCalcResultList[].term").type(JsonFieldType.NUMBER)
                                        .description("기간"),
                                fieldWithPath("data.dtiCalcResultList[].annualPrincipalRepayment").type(JsonFieldType.NUMBER)
                                        .description("연간 원금 상환액"),
                                fieldWithPath("data.dtiCalcResultList[].annualInterestRepayment").type(JsonFieldType.NUMBER)
