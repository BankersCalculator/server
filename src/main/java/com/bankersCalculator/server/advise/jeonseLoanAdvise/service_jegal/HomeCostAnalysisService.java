...
A. HomeCostAnalysisService.java : 주거비용분석서비스.
 a) 전세 1개 투입 (전세대출추천)
 b) 전세 2개 투입 (전세대출추천, 비교)
 c) 전세, 월세 각 1개투입 (전세대출추천, 비교)
   가. 전월세 단순비용 계산
     - 이자금액
     - 부수비용 (보증료, 인지세 등)
...
B. JeonseLoanAdvisorService.java : 전세대출추천서비스.
 @ return 상품분류, 상품코드, 상품명, 선정기준, 순위, 필요기준 금액/이자율/매월발생이자

 0) JeonseProductFactory.java :
 ...
 1) ProductFilteringService.java : 입력정보를 바탕으로 충족/미충족 여부를 판단. 충족/미충족 여부에 대해 반환.
              (입력정보와 상품팩토리의 상품별 요구조건 비교 후 그 결과를 반환)
   @param 사용자입력값 (고객정보, 주택정보)
   @return 상품분류, 상품코드, 상품명, 가능여부
          (JUNSE, HF0001, 신생아특버팀목전제자금대출, Y)
   ex) A의 입력정보를 바탕으로 가능한 상품은 가,나,다 입니다. 라, 마는 z 사유에 의해 불가합니다.

 2) LoanLimitCostEstimatorService.java,  대출한도/비용산출 : 필터링 통과 상품의 한도 및 비용산출
    min(필요금액, 최대금액) * 이자율 = 매월발생이자
    @return 최대가능기준 금액/이자율/매월발생이자,  필요기준 금액/이자율/매월발생이자, 발생비용

 3) ProductSelectionService.java, 상품선정 (해당 부분의 고도화는 추후 생각하는 것으로하고 최대한도, 최저금리 기준으로만 선정)
    가. 최대한도기준
    나. 최저금리기준
    다. 이용가능연수 기준
    (...)
    최대한도기준 -  1순위, 최저금리기준 - 1순위 총 2건의 상품선정
    @return 순위, 선정기준
